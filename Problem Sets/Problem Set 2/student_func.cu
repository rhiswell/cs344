                      ...
                      unsigned char* const blueChannel)
{
  ...
  uchar4 pixelColor = inputImageRGBA[thread_1D_pos];
  redChannel[thread_1D_pos]   = pixelColor.x;
  greenChannel[thread_1D_pos] = pixelColor.y;
  blueChannel[thread_1D_pos]  = pixelColor.z;
}
                        ...
                        const int filterWidth)
{
  ...
  int gridWidth  = (numCols + blockSize.x - 1) / blockSize.x;
  int gridHeight = (numRows + blockSize.y - 1) / blockSize.y;

  const dim3 gridSize(gridWidth, gridHeight);

  //std::cout << "grid_w, grid_h, block_w, block_h" << std::endl
